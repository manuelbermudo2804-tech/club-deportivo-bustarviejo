...
Deno.serve(async (req) => {
  try {
    ...
    // 🔒 Tope de cierre del bracket: 28 jun 2026 16:00 UTC (= 18:00 Europe/Madrid CEST).
    ...
    if (Date.now() > BRACKET_DEADLINE_MS) {
      ...
      return Response.json({ error: 'Fecha límite del bracket superada (18:00) — porra bloqueada' }, { status: 403 });
    }
