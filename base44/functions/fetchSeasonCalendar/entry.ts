    ...
    for (let j = 1; j <= totalJornadas; j++) {
      try {
        ...
        const records = matches.map(m => ({
          ...
          goles_local: m.jugado ? m.goles_local : undefined,
          goles_visitante: m.jugado ? m.goles_visitante : undefined,
          estado: m.jugado ? 'finalizado' : 'pendiente',
          fecha_partido: m.fecha || undefined,
