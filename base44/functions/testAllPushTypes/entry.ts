...
Deno.serve(async (req) => {
  try {
    ...
    // All 15 push notification types from onPushTrigger
    const testMessages = [
    ...
    for (let i = 0; i < testMessages.length; i++) {
      ...
      const payload = JSON.stringify({
        ...
        body: msg.body,
        tag: msg.tag,
        badgeCount: testMessages.length - i,
        renotify: true,
    ...
    return Response.json({ success: true, email, types_sent: results.length, results });
  } catch (error) {
