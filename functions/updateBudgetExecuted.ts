...
function parseSeasonRange(seasonStr, fallbackStart, fallbackEnd) {
  try {
    ...
    // Suponemos temporada Sep 1 a Jun 30 si no hay fechas en SeasonConfig
    const start = new Date(y1, 8, 1); // Sep 1, y1
    const end = new Date(y2, 5, 30, 23, 59, 59, 999); // Jun 30, y2
    return { start, end };
  } catch {
