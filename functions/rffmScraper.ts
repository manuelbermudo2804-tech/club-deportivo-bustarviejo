...
function parseJornadaMatches(html) {
  ...
  for (let i = 2; i < tables.length; i++) {
    ...
    // Extract acta URL: look for NFG_CmpPartido link in the entire match table
    let actaUrl = null;
    const tableLinks = $(table).find('a[href*="NFG_CmpPartido"]');
    if (tableLinks.length > 0) {
      const href = $(tableLinks[0]).attr('href') || '';
  ...
  // Fallback: if no actas found in tables, try extracting all NFG_CmpPartido links from page
  // and match them to played matches in order
  const matchesWithoutActa = matches.filter(m => m.jugado && !m.acta_url);
  if (matchesWithoutActa.length > 0) {
    const allActaLinks = [];
    const reActa = /href="([^"]*NFG_CmpPartido[^"]*)"/gi;
    let ma;
    while ((ma = reActa.exec(html)) !== null) {
      let href = ma[1];
      if (!href.startsWith('http')) href = `https://intranet.ffmadrid.es${href.startsWith('/') ? '' : '/nfg/NPcd/'}${href}`;
      allActaLinks.push(href);
    }
    // The acta links appear in the same order as matches on the page
    // Assign them to played matches in order
    let actaIdx = 0;
    for (const m of matches) {
      if (m.jugado && !m.acta_url && actaIdx < allActaLinks.length) {
        m.acta_url = allActaLinks[actaIdx];
        actaIdx++;
      }
    }
  }
