...
export default function PlayerProfile() {
  ...
  const { data: player, isLoading } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      // Intento 1: por player_id si existe
      if (user?.player_id) {
        const byId = await base44.entities.Player.filter({ id: user.player_id });
        if (byId?.length) return byId[0];
      }
      // Intento 2: por email_jugador (patrón usado en el Layout)
      const arr = await base44.entities.Player.filter({ email_jugador: user.email, acceso_jugador_autorizado: true, activo: true }, "-updated_date", 1);
      return arr?.[0] || null;
    },
    initialData: null,
  });
  ...
  const handleSave = () => {
    if (!player) return;
    const payload = {
      // No modificamos 'deporte' (categoría) – solo lectura
      nombre: form.nombre,
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-6">
        <Card>
          ...
          <CardContent className="space-y-3 text-slate-700">
            ...
            <p className="text-sm">Contacta con tu coordinador para activar el acceso de jugador o vincular tu ficha.</p>
          </CardContent>
        </Card>
      </div>
    );
