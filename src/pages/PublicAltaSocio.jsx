...
export default function PublicAltaSocio() {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [tipoPago, setTipoPago] = useState("unico");

  const urlParams = new URLSearchParams(window.location.search);
  const paidOk = urlParams.get("paid") === "ok";
        ...
        <Card className="max-w-md w-full border-none shadow-2xl bg-gradient-to-br from-green-100 to-green-200">
          <CardContent className="pt-8 pb-8">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <Button onClick={() => window.location.href = getShareUrl()} className="mt-4 bg-green-600 hover:bg-green-700 text-white">
                Volver al inicio
              </Button>
            </div>
          </CardContent>
        ...
        <Card className="max-w-md w-full border-none shadow-2xl bg-white">
          <CardContent className="pt-8 pb-8">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <Button onClick={() => window.location.href = getShareUrl()} className="mt-4 bg-orange-600 hover:bg-orange-700 text-white">
        ...
        <Card className="border-none shadow-xl bg-white/95 backdrop-blur-sm overflow-hidden relative">
          ...
          <CardContent className="relative space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                { icon: Heart, color: "green", title: "Apoyo esencial al club", desc: "Tu aportación es vital para el desarrollo de nuestros jóvenes deportistas" },
                { icon: Users, color: "orange", title: "Fuerza para la comunidad", desc: "Únete a la gran familia del club y vive la pasión por el deporte en Bustarviejo" },
                { icon: PartyPopper, color: "blue", title: "Eventos inolvidables", desc: "Participa en las actividades del club y comparte experiencias únicas" },
                { icon: Sparkles, color: "purple", title: "Compromiso con el deporte base", desc: "Contribuye al crecimiento y formación de nuestros deportistas" },
              ].map((item, i) => (
                <div key={i} className={`flex items-start gap-3 bg-gradient-to-r from-${item.color}-50 to-${item.color}-100 rounded-xl p-3 shadow-sm`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="p-4 rounded-xl border-2 bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <Label htmlFor="referido_por" className="font-semibold text-purple-900">🎁 ¿Quién te ha invitado?</Label>
                    <p className="text-xs text-purple-700 mt-1 mb-2">Si un amigo te invitó, escribe su nombre (opcional)</p>
                    <Input id="referido_por" value={formData.referido_por} onChange={(e) => handleChange("referido_por", e.target.value)} placeholder="Nombre de quien te invitó (opcional)" className="bg-white" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="nombre" value={formData.nombre_completo} onChange={(e) => handleChange("nombre_completo", e.target.value)} placeholder="Juan García López" required />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="dni">DNI</Label>
                    <Input id="dni" value={formData.dni} onChange={(e) => handleChange("dni", e.target.value)} placeholder="12345678A" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="telefono">Teléfono Móvil</Label>
                    <Input id="telefono" type="tel" value={formData.telefono} onChange={(e) => handleChange("telefono", e.target.value)} placeholder="600123456" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="email">Correo Electrónico *</Label>
                    <Input id="email" type="email" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} placeholder="correo@ejemplo.com" required />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="fecha_nacimiento">Fecha de Nacimiento</Label>
                    <Input id="fecha_nacimiento" type="date" value={formData.fecha_nacimiento} onChange={(e) => handleChange("fecha_nacimiento", e.target.value)} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="municipio">Municipio</Label>
                    <Input id="municipio" value={formData.municipio} onChange={(e) => handleChange("municipio", e.target.value)} placeholder="Bustarviejo" />
                  </div>
                  <div className="space-y-2 md:col-span-2">
                    <Label htmlFor="direccion">Dirección Completa</Label>
                    <Input id="direccion" value={formData.direccion} onChange={(e) => handleChange("direccion", e.target.value)} placeholder="Calle, número, piso..." />
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <RadioGroup value={tipoPago} onValueChange={setTipoPago} className="space-y-2">
                    <div className="flex items-center space-x-3 p-4 bg-white rounded-xl border-2 border-slate-200 hover:border-green-400 transition-colors">
                      <RadioGroupItem value="unico" id="pago_unico" />
                      <Label htmlFor="pago_unico" className="cursor-pointer flex-1">
                    ...
                    <div className="flex items-center space-x-3 p-4 bg-white rounded-xl border-2 border-slate-200 hover:border-green-400 transition-colors">
                      <RadioGroupItem value="suscripcion" id="pago_suscripcion" />
                      <Label htmlFor="pago_suscripcion" className="cursor-pointer flex-1">
