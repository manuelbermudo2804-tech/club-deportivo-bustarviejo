...
export default function ParentChat() {
  ...
  return (
    <div className={`${isMobile ? 'h-screen flex flex-col overflow-hidden' : 'p-4 lg:p-6 min-h-screen bg-slate-50'}`}>
      {/* MÓVIL: Lista de equipos (sin chat visible) */}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="bg-gradient-to-r from-cyan-600 to-cyan-700 text-white flex-shrink-0">
            ...
            <div className="flex border-t border-cyan-500/30">
              <button
                onClick={() => setCoordinationTab("chat")}
                className={`flex-1 py-3 px-4 text-sm font-bold flex items-center justify-center gap-2 ${
                  coordinationTab === "chat" ? "bg-white text-cyan-700" : "text-white/80"
                }`}
              >
                <Lock className="w-4 h-4" />
                💬 Chat
              ...
              <button
                onClick={() => setCoordinationTab("anuncios")}
                className={`flex-1 py-3 px-4 text-sm font-bold flex items-center justify-center gap-2 ${
                  coordinationTab === "anuncios" ? "bg-white text-cyan-700" : "text-white/80"
                }`}
              >
                📢 Anuncios
              </button>
          ...
          <div className="flex-1 overflow-hidden">
            {coordinationTab === "chat" ? (
              ...
              <div className="h-full overflow-y-auto p-3 space-y-3" style={{ backgroundColor: '#e5ddd5' }}>
                {currentAnnouncements.length === 0 ? (
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center text-slate-500 bg-white/80 rounded-xl p-6">
                      <AlertCircle className="w-12 h-12 mx-auto mb-2 opacity-50" />
                    ...
                    item.type === 'date' ? (
                      ...
                      <div key={item.data.id} className="flex justify-start">
                        <div className="max-w-[85%] bg-white rounded-2xl shadow-md overflow-hidden rounded-bl-sm">
                          <div className="px-3 py-2">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-xs font-bold text-cyan-700">🎓 {item.data.remitente_nombre || "Coordinador"}</span>
                              {item.data.prioridad !== "Normal" && (
                                <Badge className={item.data.prioridad === "Urgente" ? "bg-red-500" : "bg-yellow-500"}>
                                  {item.data.prioridad}
                                </Badge>
