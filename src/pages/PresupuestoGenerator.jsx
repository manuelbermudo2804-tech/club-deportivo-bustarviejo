...
// Compartimos branding (sello/firma/logo) con factura y recibo
const LS_SELLO = "recibo_sello_url";
const LS_LOGO = "recibo_logo_url";
const LS_FIRMA = "recibo_firma_url";
// Reutilizamos también datos del emisor de la factura
...
export default function PresupuestoGenerator() {
  ...
  const [form, setForm] = useState({
    ...
    iban: "",
    observaciones: "Presupuesto sin compromiso. Confirmación necesaria por escrito para iniciar los servicios.",
  });
  ...
  const uploadImage = async (file, type) => {
    ...
    try {
      ...
      if (type === "logo") { setLogoUrl(file_url); localStorage.setItem(LS_LOGO, file_url); await saveBranding("logo_url", file_url); toast.success("Logo actualizado"); }
      else if (type === "firma") { setFirmaUrl(file_url); localStorage.setItem(LS_FIRMA, file_url); await saveBranding("firma_url", file_url); toast.success("Firma guardada"); }
      else { setSelloUrl(file_url); localStorage.setItem(LS_SELLO, file_url); await saveBranding("sello_url", file_url); toast.success("Sello guardado"); }
    } catch {
  ...
  const handleShareWhatsApp = async () => {
    ...
    setSharingWA(true);
    try {
      const { blob, filename } = await generatePresupuestoBlob(buildParams());
  ...
  const resetSello = () => { localStorage.removeItem(LS_SELLO); setSelloUrl(""); saveBranding("sello_url", ""); };
  const resetFirma = () => { localStorage.removeItem(LS_FIRMA); setFirmaUrl(""); saveBranding("firma_url", ""); };
    ...
    <div className="min-h-screen bg-slate-50 p-4 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl lg:text-3xl font-black text-slate-900">Generador de Presupuestos</h1>
            <p className="text-sm text-slate-500">Presupuestos profesionales con validez, sello y firma</p>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">Sello, firma y logo se comparten con facturas y recibos.</p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <Input type="date" value={form.fechaValidez} onChange={(e) => setForm({ ...form, fechaValidez: e.target.value })} />
                  </div>
                </div>
              ...
              <CardContent className="space-y-3">
                  ...
                  <div key={idx} className="border border-slate-200 rounded-lg p-3 space-y-2 bg-slate-50">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <Input type="number" step="0.01" value={linea.cantidad} onChange={(e) => updateLinea(idx, "cantidad", e.target.value)} />
                      </div>
                      <div>
                        <Label className="text-xs">Precio unitario (€)</Label>
                        <Input type="number" step="0.01" value={linea.precio} onChange={(e) => updateLinea(idx, "precio", e.target.value)} placeholder="500.00" />
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <div className="grid grid-cols-2 gap-3 pt-2 border-t">
                  <div>
                    ...
                    <Input type="number" step="0.01" value={form.ivaPct} onChange={(e) => setForm({ ...form, ivaPct: e.target.value })} />
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <Input value={telefonoWA} onChange={(e) => setTelefonoWA(e.target.value)} placeholder="600 123 456 (sin prefijo si es España)" />
              </CardContent>
            </Card>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Button onClick={handleDownload} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-12 text-base font-bold">
          ...
          <div className="lg:sticky lg:top-6 self-start">
            <Card>
              ...
              <CardContent>
                <PresupuestoPreview {...form} {...emisor} lineas={lineas} logoUrl={logoUrl} selloUrl={selloUrl} firmaUrl={firmaUrl} />
              </CardContent>
            </Card>
          </div>
        </div>
