...
export default function FacturaGenerator() {
  ...
  const uploadImage = async (file, type) => {
    ...
    try {
      ...
      if (type === "logo") { setLogoUrl(file_url); localStorage.setItem(LS_LOGO, file_url); await saveBranding("logo_url", file_url); toast.success("Logo actualizado para todos los dispositivos"); }
      else if (type === "firma") { setFirmaUrl(file_url); localStorage.setItem(LS_FIRMA, file_url); await saveBranding("firma_url", file_url); toast.success("Firma guardada para todos los dispositivos"); }
      else { setSelloUrl(file_url); localStorage.setItem(LS_SELLO, file_url); await saveBranding("sello_url", file_url); toast.success("Sello guardado para todos los dispositivos"); }
    } catch {
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">Sello, firma y logo se guardan en la nube y se sincronizan en todos los dispositivos del club.</p>
                </div>
              </CardContent>
            </Card>
