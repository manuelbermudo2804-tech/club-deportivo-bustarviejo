...
export default function IndividualReportDialog({ 
  ...
  const handleSend = () => {
    ...
    onSend({ player, dateRange, sendMethod, periodType });
  };
      ...
      <DialogContent className="max-w-md">
          ...
          <div className="bg-slate-50 p-3 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">{player?.categoria_principal || player?.deporte}</p>
              </div>
            </div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSend} disabled={isLoading} className="bg-orange-600 hover:bg-orange-700">
            {isLoading ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Enviando...</>) : (<><Send className="w-4 h-4 mr-2" />Enviar Reporte</>)}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
