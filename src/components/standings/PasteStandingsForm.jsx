...
function parseStandingsText(raw) {
  ...
  // Detectar temporada y grupo de las primeras líneas
  let temporada = "";
  let grupo = "";
  let headerEnd = 0;
  ...
  for (let i = 0; i < Math.min(lines.length, 6); i++) {
    const l = lines[i];
    // Temporada: "2025-2026" o "2025/2026"
    if (/^\d{4}[-\/]\d{4}$/.test(l)) {
      temporada = l.replace('-', '/');
      headerEnd = i + 1;
      continue;
    }
    // Grupo: línea con "Grupo" o competición
  ...
  // Parsear filas de datos: buscar líneas que empiecen con un número (posición)
  const standings = [];
  ...
  while (i < dataLines.length) {
    ...
    // ¿Empieza con un número (posición)?
    const posMatch = line.match(/^(\d{1,2})$/);
    if (posMatch && i + 1 < dataLines.length) {
      const posicion = parseInt(posMatch[1], 10);
      
      // La siguiente línea debería ser el nombre del equipo
      const nombreEquipo = dataLines[i + 1];
      
      // Después vienen los números: puntos, jugados, ganados, empates, perdidos, gf, gc, [sanción]
      const nums = [];
      let j = i + 2;
    ...
    // Fallback: intentar línea completa tipo "1 EQUIPO 41 18 13 2 3 50 22 0"
...
export default function PasteStandingsForm({ preselectedCategory, onDataExtracted, onCancel }) {
  ...
  const handleParse = () => {
    ...
    // Calcular jornada: max partidos jugados de cualquier equipo
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            Parsear Datos ({text.split('\n').filter(l => /^\d{1,2}$/.test(l.trim())).length} equipos detectados)
          </Button>
        </div>
      </CardContent>
    </Card>
