...
import { Heart, Phone, Clock, Sun, Sunset, Download, Search, MessageCircle, Trash2 } from "lucide-react";
...
export default function VolunteersList() {
  ...
  const counts = useMemo(() => ({
    ...
    manana: voluntarios.filter(v => v.disponibilidad_manana).length,
    tarde: voluntarios.filter(v => v.disponibilidad_tarde).length,
  }), [voluntarios]);
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [voluntarios, estadoFilter, search]);
        ...
        <div className="grid grid-cols-2 gap-2">
          <Card className="bg-yellow-50 border-yellow-200">
            <CardContent className="p-2 text-center">
              ...
              <p className="text-lg font-black text-yellow-800">{counts.manana}</p>
              <p className="text-xs text-yellow-700 font-semibold">Mañana</p>
            </CardContent>
          </Card>
          <Card className="bg-orange-50 border-orange-200">
            <CardContent className="p-2 text-center">
              <Sunset className="w-4 h-4 text-orange-600 mx-auto" />
              <p className="text-lg font-black text-orange-800">{counts.tarde}</p>
              <p className="text-xs text-orange-700 font-semibold">Tarde</p>
            </CardContent>
          </Card>
        </div>
      )}
              ...
              <Card key={v.id} className={`border-l-4 ${estado === "pendiente" ? "border-l-orange-500" : estado === "confirmado" ? "border-l-green-500" : "border-l-pink-500"}`}>
                <CardContent className="p-3 space-y-2">
                  ...
                  <div className="flex items-center gap-2 pt-1 flex-wrap">
                    ...
                    <Select
                      value={estado}
                      onValueChange={(nuevo) => updateMutation.mutate({ id: v.id, estado: nuevo })}
                    >
