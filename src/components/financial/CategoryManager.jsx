...
import { Plus, Edit2, Trash2, Save, X, Users, ArrowUp, ArrowDown } from "lucide-react";
...
export default function CategoryManager() {
  ...
  const handleMove = async (category, direction) => {
    ...
    await base44.entities.CategoryConfig.update(category.id, { ...category, orden: targetCategory.orden });
    await base44.entities.CategoryConfig.update(targetCategory.id, { ...targetCategory, orden: category.orden });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Categorías y Cuotas</h2>
          <p className="text-sm text-slate-600">Configura las categorías del club y sus cuotas por periodo</p>
        </div>
        <div className="flex gap-2">
          {categories.length === 0 && (
            <Button onClick={initializeDefaultCategories} variant="outline" size="sm">
              Inicializar por Defecto
            </Button>
          )}
          <Button onClick={handleNew} size="sm" className="bg-green-600 hover:bg-green-700">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 flex-1">
                      <div className="flex flex-col gap-1">
                        <Button variant="ghost" size="sm" onClick={() => handleMove(category, 'up')} disabled={index === 0} className="h-6 w-6 p-0">
                          <ArrowUp className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleMove(category, 'down')} disabled={index === categories.length - 1} className="h-6 w-6 p-0">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          ...
                          {playersCount > 0 && (
                            <Badge variant="outline" className="text-orange-700">{playersCount} jugadores</Badge>
                          )}
                        </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => handleDelete(category)} className="text-red-600 hover:text-red-700">
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editForm?.id ? "Editar Categoría" : "Nueva Categoría"}</DialogTitle>
            <DialogDescription>Define el nombre de la categoría y sus cuotas para cada periodo</DialogDescription>
          </DialogHeader>
          ...
          {editForm && (
            <div className="space-y-4 py-4">
              <div>
                ...
                <Input value={editForm.nombre} onChange={(e) => setEditForm({ ...editForm, nombre: e.target.value })} placeholder="Ej: Fútbol Cadete, Baloncesto Infantil" />
              </div>

              <div className="grid grid-cols-3 gap-4">
                <div>
                  <Label>Inscripción (Junio) *</Label>
                  <Input type="number" value={editForm.cuota_inscripcion} onChange={(e) => setEditForm({ ...editForm, cuota_inscripcion: parseFloat(e.target.value) || 0 })} />
                </div>
                <div>
                  <Label>Segunda (Septiembre) *</Label>
                  <Input type="number" value={editForm.cuota_segunda} onChange={(e) => setEditForm({ ...editForm, cuota_segunda: parseFloat(e.target.value) || 0 })} />
                </div>
                <div>
                  <Label>Tercera (Diciembre) *</Label>
                  <Input type="number" value={editForm.cuota_tercera} onChange={(e) => setEditForm({ ...editForm, cuota_tercera: parseFloat(e.target.value) || 0 })} />
                </div>
              </div>
              ...
              <div>
                ...
                <Textarea value={editForm.notas || ""} onChange={(e) => setEditForm({ ...editForm, notas: e.target.value })} rows={2} placeholder="Información adicional" />
              </div>

              <div className="flex items-center space-x-2 p-3 bg-slate-50 rounded border">
                <Checkbox id="activa" checked={editForm.activa} onCheckedChange={(checked) => setEditForm({ ...editForm, activa: checked })} />
