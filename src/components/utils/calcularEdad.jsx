...
export function getSuggestedCategory(edad, deporteActual) {
  ...
  if (esJugadoraFemenina || deporteActual?.includes("Femenino")) {
    if (edad >= 5 && edad <= 7) return "Fútbol Pre-Benjamín (Mixto)";
    if (edad >= 8 && edad <= 9) return "Fútbol Benjamín (Mixto)";
    if (edad >= 10 && edad <= 11) return "Fútbol Alevín (Mixto)";
    if (edad >= 12 && edad <= 13) return "Fútbol Infantil (Mixto)";
    return "Fútbol Femenino"; // A partir de 14 años
  }

  // RANGOS OFICIALES FEDERACIÓN ESPAÑOLA (2026)
  if (edad >= 5 && edad <= 7) return "Fútbol Pre-Benjamín (Mixto)";
  if (edad >= 8 && edad <= 9) return "Fútbol Benjamín (Mixto)";
  if (edad >= 10 && edad <= 11) return "Fútbol Alevín (Mixto)";
  if (edad >= 12 && edad <= 13) return "Fútbol Infantil (Mixto)";
  if (edad >= 14 && edad <= 15) return "Fútbol Cadete";
