...
export default function ChatAlertBanner({ 
  ...
  // Determinar URL destino según el rol
  const chatUrl = isAdmin 
    ? createPageUrl("AdminChat") 
    : isCoach 
      ? createPageUrl("CoachChat") 
      : createPageUrl("ParentChat");
              ...
              {unreadGroupMessages > 0 && (
                <div className="bg-white/20 backdrop-blur-sm px-3 py-1.5 rounded-full">
                  <p className="text-white text-xs font-semibold">
                    📢 {unreadGroupMessages} {unreadGroupMessages === 1 ? 'mensaje' : 'mensajes'} {isCoach || isAdmin ? 'de familias' : 'del entrenador'}
                  </p>
                </div>
              )}
              {unreadPrivateMessages > 0 && (
                <div className="bg-green-500/80 backdrop-blur-sm px-3 py-1.5 rounded-full flex items-center gap-1">
                  ...
                  <p className="text-white text-xs font-semibold">
                    {unreadPrivateMessages} {unreadPrivateMessages === 1 ? 'privado' : 'privados'}
                  </p>
                </div>
              )}
            </div>
