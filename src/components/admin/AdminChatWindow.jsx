...
export default function AdminChatWindow({ conversation, user, onClose, onMarkResolved }) {
  ...
  const sendMessageMutation = useMutation({
    ...
    mutationFn: async (messageData) => {
      await base44.entities.AdminMessage.create({
        ...
        leido_admin: true,
        leido_padre: messageData.es_nota_interna ? true : false,
        fecha_leido_admin: new Date().toISOString()
      });

      if (!messageData.es_nota_interna) {
        await base44.entities.AdminConversation.update(conversation.id, {
  ...
  const handleSendInternalNote = (messageData) => {
    sendMessageMutation.mutate({ ...messageData, es_nota_interna: true });
  };
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isMine ? 'justify-end mr-2' : 'justify-start ml-2'} mb-1.5`}>
              ...
              } rounded-2xl px-3 py-2 relative`} style={{
